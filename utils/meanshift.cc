...
void MeanShift::clustermax(std::vector<Cluster> &clusters){
    ...
    for (int i = 0; i < clusters.size(); i++) {
        ...
        for (int j = 0; j < cur.original_points.size(); j++){
            if(cur.original_points[j][0] > maxX){
                maxX = cur.original_points[j][0];
            }
            if(cur.original_points[j][0] < minX){
                minX = cur.original_points[j][0];
            }
        }
...
vector<Cluster> MeanShift::cluster(const std::vector<Point> &points, double kernel_bandwidth){
    ...
    vector<Cluster> clusters =  cluster(points, shifted_points);
    clustermax(clusters);
    return clusters;
}
